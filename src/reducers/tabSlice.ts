...
const initialState = {} as D3State as SwitchTab;
...
const tabSlice = createSlice({
    ...
    reducers: {
      ...
      closeTab (state, action: PayloadAction<SwitchTab>) {
        ...
        const newOpenFiles = openFiles.filter((file: any) => file != action.payload.filePath);
        ...
        if (action.payload.filePath === state.filePath) {
          ...
          if (openFiles.length > 1) switchTab(newOpenFiles[0], newOpenFiles);
